...
def listify_dataset(apps, schema_editor):
    logging.getLogger(MIGRATION_LOGGER).info(json.dumps({'task':'listify_dataset', 'info': 'Transforming task params dataset to datasets.'}))
    ...
    for task in Task.objects.all():
        try:
            ...
            logging.getLogger(MIGRATION_LOGGER).info(json.dumps({'task':'listify_dataset', 'new_datasets_value': params['dataset'], 'task_id': task.id, 'task_type': task.task_type}))
            task.save()
