...
class UAATests(APILiveServerTestCase):
    ...
    def create_groups(self):
        access_token, refresh_token = self.uaa_login(username=TEST_UAA_USERNAME, password=TEST_UAA_PASSWORD,
                                                     scopes="openid texta.* uaa.admin")
    ...
    def create_project1(self):
        access_token, refresh_token = self.uaa_login(username="test3", password="test3",
                                                     scopes="openid texta.ou")
    ...
    def create_project2(self):
        access_token, refresh_token = self.uaa_login(username="test2", password="test2",
                                                     scopes="openid texta.ou texta.project_admin")
    ...
    def create_project3(self):
        access_token, refresh_token = self.uaa_login(username="test1", password="test1",
                                                     scopes="openid texta.ou texta.admin texta.project_admin")
    ...
    def run_user_in_scopes_has_access_to_project_where_user_is_not_listed(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {access_token}')
        auth_resp = self.client.get(f'{TEST_VERSION_PREFIX}/', format='json')
        print_output("run_user_in_scopes_has_access_to_project_where_user_is_not_listed:auth_resp.data", auth_resp.data)
        # Check if the UaaAuthentication returned 200
        self.assertEqual(200, auth_resp.status_code)
        # Get user projects
