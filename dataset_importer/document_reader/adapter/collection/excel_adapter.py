...
class ExcelAdapter(CollectionAdapter):
    ...
    def get_features(**kwargs):
        ...
        for file_extension in ['xls', 'xlsx']:
            for file_path in ExcelAdapter.get_file_list(directory, file_extension):
                print(file_path)
                ...
                for column_idx in range(sheet.ncols):
                    col_types = list({col_type for col_type in sheet.col_types(column_idx)[1:]  # TODO [1:] will go out of range
                                 if col_type not in ExcelAdapter.empty_and_blank_codes})
                    col_values = sheet.col_values(column_idx)
                    feature_converters.append(ExcelAdapter.get_column_converter(col_types, col_values))

                for row_idx, excel_row in ((row_idx, sheet.row(row_idx)) for row_idx in range(1, sheet.nrows)):
                    document = {feature_labels[col_idx]: feature_converters[col_idx](cell.value)
    ...
    def get_column_converter(value_types, values):
        if len(value_types) == 1:
            code = value_types[0]
            if code == 1:
                return ExcelAdapter.converters['text']
            elif code == 2:
                if all(isinstance(value, int) for value in values):
                    return ExcelAdapter.converters['int']
                else:
                    return ExcelAdapter.converters['float']
            elif code == 3:
                return ExcelAdapter.converters['date']
            elif code == 4:
                return ExcelAdapter.converters['bool']
            else:
                return ExcelAdapter.converters['default']
